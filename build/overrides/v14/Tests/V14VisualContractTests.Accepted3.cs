...
internal static class V14VisualContractTests
{
    ...
    internal static void Run()
    {
        ...
        if (!thread.Join(TimeSpan.FromSeconds(20)))
            throw new TimeoutException("v1.4 WPF visual contracts did not finish");
        if (failure is not null)
            throw new InvalidOperationException("v1.4 WPF visual contract failed", failure);
    }
    ...
    private static void SystemReferenceRows()
    {
        ...
        var card = content.View as WpfBorder
        ...
        True(rows.Children[4] is WpfBorder);
    ...
    private static void TransportCorners()
    {
        ...
        foreach (var button in buttons)
        {
            ...
            Equal(typeof(WpfBorder), factory.Type);
