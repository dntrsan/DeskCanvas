...
internal static class SpectrumMath
{
    ...
    private const double DigitalSilenceDb = -110d;

    internal static void Fft(Span<double> real, Span<double> imaginary)
    {
        for (var i = 1, j = 0; i < real.Length; i++)
        {
            var bit = real.Length >> 1;
            for (; (j & bit) != 0; bit >>= 1) j ^= bit;
            j ^= bit;
            if (i < j) (real[i], real[j]) = (real[j], real[i]);
            if (i < j) (imaginary[i], imaginary[j]) = (imaginary[j], imaginary[i]);
        }
    ...
    internal static double[] ToLogBands(ReadOnlySpan<double> real, ReadOnlySpan<double> imaginary, int sampleRate)
    {
        ...
        var dbValues = new double[BandCount];
        var half = real.Length / 2;
        var min = 45d;
        var max = Math.Min(18000d, sampleRate / 2d);
        for (var band = 0; band < BandCount; band++)
        {
            ...
            dbValues[band] = 20 * Math.Log10(Math.Max(magnitude, 1e-12));
        }

        var strongestBandDb = dbValues.Max();
        if (!double.IsFinite(strongestBandDb) || strongestBandDb < DigitalSilenceDb)
        {
            return values;
        }
        ...
        for (var band = 0; band < BandCount; band++)
        {
            // The visualizer represents the frequency shape, not speaker volume.  A
            // quiet valid frame therefore has the same contour as a loud equivalent.
            values[band] = Math.Clamp((dbValues[band] - strongestBandDb + 36d) / 36d, 0d, 1d);
        }

        return values;
    }

    internal static double[] AnalyzeSamples(ReadOnlySpan<double> input, int sampleRate)
    {
        if (input.Length == 0 || input.All(static sample => !double.IsFinite(sample) || Math.Abs(sample) < 1e-9))
        {
            return new double[BandCount];
        }

        var real = new double[input.Length];
        var imaginary = new double[input.Length];
        for (var i = 0; i < input.Length; i++)
        {
            real[i] = input[i] * (.5 - .5 * Math.Cos(2 * Math.PI * i / (input.Length - 1)));
        }

        Fft(real, imaginary);
        return ToLogBands(real, imaginary, sampleRate);
    }

    // Keep the existing attack/release character after relative normalization.
    internal static double Smooth(double prior, double target) => target >= prior
        ? prior + (target - prior) * .62
        : prior + (target - prior) * .16;
}
...
internal sealed class WasapiLoopbackSpectrumReader : IAudioSpectrumReader
{
    ...
    public IReadOnlyList<double> ReadBands()
    {
        lock (gate) return bands.ToArray();
    }
    ...
    private void CaptureSession()
    {
        ...
        try
        {
            ...
            ThrowIfFailed(((IMMDeviceEnumerator)enumerator).GetDefaultAudioEndpoint(EDataFlow.eRender, ERole.eMultimedia, out var endpoint));
            device = endpoint;
            var clientId = typeof(IAudioClient).GUID;
            ThrowIfFailed(((IMMDevice)device).Activate(ref clientId, 23, IntPtr.Zero, out var audioClient));
            client = audioClient;
            ThrowIfFailed(((IAudioClient)client).GetMixFormat(out format));
            var wave = Marshal.PtrToStructure<WaveFormatEx>(format);
            var sampleFormat = AudioSampleFormatMath.Resolve(wave.wFormatTag, wave.wBitsPerSample,
                wave.wFormatTag == AudioSampleFormatMath.Extensible ? Marshal.PtrToStructure<Guid>(format + 24) : null);
            if (sampleFormat == AudioSampleFormat.Unsupported) throw new NotSupportedException("Unsupported WASAPI mix format");
            ...
            ThrowIfFailed(((IAudioClient)client).GetService(ref captureId, out var captureClient));
            ...
            while (!cancellation.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    else AppendSamples(data, frames, wave, sampleFormat, samples);
                }
                finally { ((IAudioCaptureClient)capture).ReleaseBuffer(frames); }

                while (samples.Count >= 2048)
                {
                    var block = samples.Take(2048).ToArray();
                    samples.RemoveRange(0, 1024);
                    Publish(SpectrumMath.AnalyzeSamples(block, wave.nSamplesPerSec));
                }
            }
        ...
        {
            try { if (client is IAudioClient audio) audio.Stop(); } catch { }
    ...
    private static void AppendSamples(IntPtr data, uint frames, WaveFormatEx wave, AudioSampleFormat sampleFormat, List<double> output)
    {
        var channels = Math.Max(1, (int)wave.nChannels);
        var bytes = Math.Max(1, wave.nBlockAlign / channels);
        for (var frame = 0; frame < frames; frame++)
        {
            ...
            for (var channel = 0; channel < channels; channel++)
            {
                ...
                sum += sampleFormat == AudioSampleFormat.Float32
                    ? Marshal.PtrToStructure<float>(pointer)
                    : Marshal.ReadInt16(pointer) / 32768d;
            }
    ...
    private void Publish(double[] next)
    {
        lock (gate)
        {
            for (var i = 0; i < bands.Length; i++) bands[i] = SpectrumMath.Smooth(bands[i], next[i]);
        }
    }
    ...
    public void Dispose()
    {
        ...
        disposed = true;
        cancellation.Cancel();
        if (worker.Join(750)) cancellation.Dispose();
    }
    ...
    [Flags] private enum AudioClientStreamFlags { Loopback = 0x20000 }
    [ComImport, Guid("BCDE0395-E52F-467C-8E3D-C4579291692E")] private class MMDeviceEnumeratorComObject { }
    [ComImport, Guid("A95664D2-9614-4F35-A746-DE8DB63617E6"), InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
    private interface IMMDeviceEnumerator
    {
        int EnumAudioEndpoints(EDataFlow flow, int state, out IntPtr collection);
    ...
    {
        int Activate(ref Guid iid, int cls, IntPtr parameters, [MarshalAs(UnmanagedType.IUnknown)] out object instance);
    }

    [ComImport, Guid("1CB9AD4C-DBFA-4c32-B178-C2F568A703B2"), InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
    private interface IAudioClient
    {
        int Initialize(AudioClientShareMode mode, AudioClientStreamFlags flags, long duration, long period, IntPtr format, IntPtr session);
        ...
        int Start(); int Stop(); int Reset(); int SetEventHandle(IntPtr handle);
...
{
    private readonly Func<IAudioSpectrumReader> factory;
    private IAudioSpectrumReader? reader;
    ...
    internal void Poll(bool playing, Action<IReadOnlyList<double>> sink)
    {
        ...
        try { reader ??= factory(); sink(reader.ReadBands()); }
        catch { Suspend(); sink(new double[SpectrumMath.BandCount]); }
    }
...
internal interface IAudioPeakReader : IDisposable { double ReadPeak(); }
internal static class AudioPeakMath
{
    internal static double Clamp(double value) => double.IsFinite(value) ? Math.Clamp(value, 0, 1) : 0;
    internal static double BarHeight(double peak, int index)
    {
        var contour = new[] { .58, .74, .91, 1d, .84, .68, .52 }[Math.Clamp(index, 0, 6)];
        return peak <= .002 ? 5 : Math.Clamp(5 + Clamp(peak) * contour * 17, 5, 22);
    }
}
