...
internal sealed class BuiltInContentPicker : Window
{
    ...
    internal BuiltInContentPicker(Window? owner, INowPlayingService nowPlaying, ISystemMetricsService metrics)
    {
        ...
        kind.SelectionChanged += (_, _) => RebuildPreview(); theme.SelectionChanged += (_, _) => ApplyOptions(); clockStyle.SelectionChanged += (_, _) => ApplyOptions(); progressStyle.SelectionChanged += (_, _) => ApplyOptions();
    ...
    private void RebuildPreview()
    {
        ...
        try { ApplyOptions(); preview?.SetActive(false); preview?.Dispose(); preview = SelectedKind switch { CanvasContentKinds.NowPlaying => new NowPlayingItemContent(previewItem, nowPlaying), CanvasContentKinds.SystemMonitor => new SystemMonitorItemContent(previewItem, metrics), _ => new ClockItemContent(previewItem) }; var size = SelectedKind switch { CanvasContentKinds.NowPlaying => new Size(360, 220), CanvasContentKinds.SystemMonitor => new Size(320, 210), _ => new Size(300, 150) }; var presenter = new Grid { Width = size.Width, Height = size.Height }; presenter.Children.Add(preview.View); previewHost.Content = presenter; preview.SetActive(true); BuildOptions(); }
        finally { rebuilding = false; }
    }
    private void BuildOptions()
    {
        options.Children.Clear(); options.Children.Add(Section($"{(kind.SelectedItem as KindChoice)?.Name} の設定"));
        if (SelectedKind == CanvasContentKinds.Clock) { options.Children.Add(Section("スタイル", new Thickness(0, 14, 0, 0))); options.Children.Add(clockStyle); options.Children.Add(use24); options.Children.Add(showSeconds); options.Children.Add(showYear); options.Children.Add(showMonthDay); }
    ...
    {
        previewItem.ContentKind = SelectedKind; previewItem.Theme = Theme; previewItem.Clock.Style = clockStyle.SelectedItem is ClockStyle style ? style : ClockStyle.Digital; previewItem.Clock.Use24Hour = use24.IsChecked == true; previewItem.Clock.ShowSeconds = showSeconds.IsChecked == true; previewItem.Clock.ShowYear = showYear.IsChecked == true; previewItem.Clock.ShowMonthDay = showMonthDay.IsChecked == true;
