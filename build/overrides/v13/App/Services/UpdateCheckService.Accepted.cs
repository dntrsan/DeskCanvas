...
internal sealed class GitHubReleaseFeed(HttpClient client) : IReleaseFeed
{
    private static readonly Uri Endpoint =
        new("https://api.github.com/repos/dntrsan/DeskCanvas/releases");
    ...
    public async Task<ReleaseInfo?> GetLatestStableAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var release in json.RootElement.EnumerateArray())
        {
            ...
            var page = pageProperty.GetString();
            if (SemanticVersion.TryParse(tag, out _) &&
                Uri.TryCreate(page, UriKind.Absolute, out var uri) &&
                UpdateSafety.IsAllowedReleasePage(uri))
            {
                return new ReleaseInfo(tag!, uri);
            }
        }
        return null;
    }
...
internal sealed class UpdateCheckService(IReleaseFeed feed, Version current)
{
    ...
    internal async Task<ReleaseInfo?> CheckOnceAsync(CancellationToken cancellationToken)
    {
        ...
        {
            var latest =
                await feed.GetLatestStableAsync(cancellationToken).ConfigureAwait(false);
