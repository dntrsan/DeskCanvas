...
{
    internal static NowPlayingSnapshot Empty { get; } = new(
        false, "", "再生中のコンテンツはありません", "", "", null,
        NowPlayingState.None, TimeSpan.Zero, TimeSpan.Zero,
...
internal sealed class NowPlayingService : INowPlayingService
{
    ...
    private bool ShouldRun
    {
        get
        {
            lock (stateGate) return !disposed && consumers > 0;
        }
    }
    ...
    private void Session_MediaPropertiesChanged(object? sender, EventArgs args) =>
        _ = RefreshAsync();
    private void Session_PlaybackInfoChanged(object? sender, EventArgs args) =>
        _ = RefreshAsync();
    private void Session_TimelinePropertiesChanged(object? sender, EventArgs args) =>
        _ = RefreshAsync();
    ...
    private async Task SwitchSessionAsync()
    {
        ...
        try
        {
            if (ShouldRun && manager is not null)
                await SwitchSessionCoreAsync().ConfigureAwait(false);
        }
        catch (Exception)
        {
            // A non-null same-media reconnect keeps the last good snapshot.
        }
        finally
        {
            gate.Release();
    ...
    private async Task RefreshAsync()
    {
        ...
        try
        {
            if (ShouldRun && session is not null)
                await RefreshCoreAsync().ConfigureAwait(false);
        }
        catch (Exception)
        {
            // GSMTC reads are transiently fallible; preserve the last good identity/timeline.
        }
        finally
        {
            gate.Release();
    ...
    private async Task RefreshCoreAsync()
    {
        ...
        if (!string.Equals(identity, mediaIdentity, StringComparison.Ordinal))
        {
            // Only a confirmed different identity (or a null session above) clears the
            // retained timeline. A same-media session reconnect never flashes empty.
            if (mediaIdentity is not null) Clear();
            mediaIdentity = identity;
            lastKnownEnd = rawEnd;
        }
        else if (rawEnd > TimeSpan.Zero)
        {
            lastKnownEnd = rawEnd;
        }

        var end = rawEnd > TimeSpan.Zero ? rawEnd : lastKnownEnd;
        var position = data.Position < TimeSpan.Zero ? TimeSpan.Zero : data.Position;
        if (end > TimeSpan.Zero && position > end) position = end;
        Publish(new NowPlayingSnapshot(
            true, source, title, artist, album, data.Artwork, data.State,
            position, end, data.CanPrevious, data.CanPlayPause, data.CanNext,
            data.CanSeek, DateTimeOffset.UtcNow));
    }

    internal static NowPlayingSnapshot Project(NowPlayingSnapshot value, DateTimeOffset now)
    {
        if (!value.HasSession ||
            value.State != NowPlayingState.Playing ||
            value.End <= TimeSpan.Zero ||
            value.ObservedAt == default)
            return value;
        var elapsed = now - value.ObservedAt;
        if (elapsed <= TimeSpan.Zero) return value;
        var projected = value.Position + elapsed;
        if (projected > value.End) projected = value.End;
        return value with { Position = projected };
    }
    ...
    public Task<bool> SeekAsync(TimeSpan position) =>
        InvokeAsync(NowPlayingCommand.Seek, position);
        ...
        TimeSpan position = default)
    {
        ...
        if (current is null || !ReferenceEquals(current, session) ||
            !IsEnabled(currentSnapshot, command))
            return false;
        if (command == NowPlayingCommand.Seek)
            position = TimeSpan.FromTicks(
                Math.Clamp(position.Ticks, 0, currentSnapshot.End.Ticks));
        try
        {
            var result = await current.TryCommandAsync(command, position)
                .ConfigureAwait(false);
            if (result && ReferenceEquals(current, session)) _ = RefreshAsync();
            return result;
        }
        catch (Exception)
        {
...
internal sealed class GsmtcSessionAdapter : INowPlayingSession
{
    ...
    public async Task<NowPlayingData> ReadAsync()
    {
        ...
        {
            // Artwork failure does not invalidate metadata/timeline.
        }
    ...
    public async Task<bool> TryCommandAsync(
        NowPlayingCommand command,
        TimeSpan position = default) =>
        command switch
        {
