...
internal sealed class BuiltInContentPicker : Window
{
    ...
    private readonly CanvasItem previewItem = new()
    {
    ...
    private readonly ContentControl previewHost = new()
    {
        ...
        ISystemMetricsService metrics)
    {
        ...
        Background = new System.Windows.Media.SolidColorBrush(
            System.Windows.Media.Color.FromRgb(17, 18, 26));
        ...
        theme.ItemsSource = Enum.GetValues<WidgetThemeKind>();
        theme.SelectedItem = WidgetThemeKind.Auto;
        clockStyle.ItemsSource = Enum.GetValues<ClockStyle>();
        clockStyle.SelectedItem = ClockStyle.Digital;
        ...
        foreach (var check in new[]
                 {
                     use24, showSeconds, showYear, showMonthDay,
                     art, timeline, sourceApp, cpu, memory, gpu, network
                 })
        {
            check.Checked += (_, _) => ApplyOptions();
            check.Unchecked += (_, _) => ApplyOptions();
        }

        var selector = new StackPanel { Margin = new Thickness(18) };
        selector.Children.Add(Section("種類"));
        selector.Children.Add(kind);
        selector.Children.Add(Section("テーマ", new Thickness(0, 18, 0, 0)));
        selector.Children.Add(theme);
        ...
        var previewBorder = new System.Windows.Controls.Border
        {
            ...
            Background = new System.Windows.Media.SolidColorBrush(
                System.Windows.Media.Color.FromRgb(22, 22, 31)),
    ...
    internal string SelectedKind =>
        (kind.SelectedItem as KindChoice)?.Kind ?? CanvasContentKinds.Clock;
    internal WidgetThemeKind Theme =>
        theme.SelectedItem is WidgetThemeKind value && Enum.IsDefined(value)
            ? value
    ...
    private void BuildOptions()
    {
        ...
        options.Children.Add(Section($"{(kind.SelectedItem as KindChoice)?.Name} の設定"));
        if (SelectedKind == CanvasContentKinds.Clock)
        {
            options.Children.Add(Section("スタイル", new Thickness(0, 14, 0, 0)));
            options.Children.Add(clockStyle);
            options.Children.Add(use24);
            options.Children.Add(showSeconds);
            options.Children.Add(showYear);
            options.Children.Add(showMonthDay);
        }
        else if (SelectedKind == CanvasContentKinds.NowPlaying)
        {
            options.Children.Add(art);
            options.Children.Add(timeline);
            options.Children.Add(sourceApp);
        }
        else
        {
            options.Children.Add(cpu);
            options.Children.Add(memory);
            options.Children.Add(gpu);
            options.Children.Add(network);
        }
    }

    private void ApplyOptions()
    {
        previewItem.ContentKind = SelectedKind;
        previewItem.Theme = Theme;
        previewItem.Clock.Style =
            clockStyle.SelectedItem is ClockStyle style ? style : ClockStyle.Digital;
        previewItem.Clock.Use24Hour = use24.IsChecked == true;
        previewItem.Clock.ShowSeconds = showSeconds.IsChecked == true;
        previewItem.Clock.ShowYear = showYear.IsChecked == true;
        previewItem.Clock.ShowMonthDay = showMonthDay.IsChecked == true;
        previewItem.NowPlaying.ShowAlbumArt = art.IsChecked == true;
        previewItem.NowPlaying.ShowTimeline = timeline.IsChecked == true;
        previewItem.NowPlaying.ShowSourceApp = sourceApp.IsChecked == true;
        previewItem.SystemMonitor.ShowCpu = cpu.IsChecked == true;
        previewItem.SystemMonitor.ShowMemory = memory.IsChecked == true;
        previewItem.SystemMonitor.ShowGpu = gpu.IsChecked == true;
        previewItem.SystemMonitor.ShowNetwork = network.IsChecked == true;
        switch (preview)
        {
    ...
    private static WpfComboBox FieldCombo() =>
        new() { Margin = new Thickness(0, 4, 0, 10) };

    private static WpfCheckBox Check(string content, bool value) =>
        new()
        {
            Content = content,
            IsChecked = value,
    ...
    private static TextBlock Section(string text, Thickness margin = default) =>
        new()
        {
            Text = text,
    ...
    private sealed record KindChoice(string Kind, string Name)
    {
