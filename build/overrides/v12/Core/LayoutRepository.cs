...
public sealed class LayoutRepository
{
    ...
    {
        Directory.CreateDirectory(root); Directory.CreateDirectory(backupDirectory);
        var temporaryPath = layoutPath + ".tmp"; File.WriteAllText(temporaryPath, JsonSerializer.Serialize(Normalize(layout), JsonOptions));
    ...
    private static CanvasLayout? TryRead(string path)
    {
        try
        {
            ...
            using var document = JsonDocument.Parse(File.ReadAllText(path)); if (document.RootElement.ValueKind != JsonValueKind.Object) return null;
