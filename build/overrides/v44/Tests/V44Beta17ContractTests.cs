...
internal static class V44Beta17ContractTests
{
    [ModuleInitializer]
    internal static void Run()
    {
        ...
        Check(ApplicationVersion.SemVer == "1.2.0-beta.17", "semver");
        Check(ApplicationVersion.Display == "v1.2.0-beta.17  BETA", "display");
        Check(ApplicationVersion.WindowTitle == "DeskCanvas v1.2.0-beta.17 Beta", "title");
        Check(informational == ApplicationVersion.SemVer, "informational");
        Check(file == "1.2.0.17", "file");
        TestBackdropMapping();
        TestPlateBlur();
        TestPassThrough();
        TestLens();
        Console.WriteLine("PASS v1.2.0-beta.17 backdrop capture contract");
    }
    ...
    {
        if (!value) throw new InvalidOperationException($"beta 17 contract failed: {name}");
    }
}
