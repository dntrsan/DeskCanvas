...
internal static class V16HardeningContractTests
{
 ...
 {
  var rate=48000;var low=Tone(100,rate),high=Tone(8000,rate);var lowBands=SpectrumMath.AnalyzeSamples(low,rate);var highBands=SpectrumMath.AnalyzeSamples(high,rate);if(Max(lowBands)>1||Max(highBands)<5)throw new InvalidOperationException("frequency bands did not separate");if(SpectrumMath.AnalyzeSamples(new double[2048],rate).Any(x=>x>.0001))throw new InvalidOperationException("silence was not zero");if(!(SpectrumMath.Smooth(.2,.8)-.2 > .8-SpectrumMath.Smooth(.8,.2)))throw new InvalidOperationException("attack was not faster than release");
 ...
 private static double[] Tone(double hz,int rate)=>Enumerable.Range(0,2048).Select(i=>Math.Sin(2*Math.PI*hz*i/rate)*.8).ToArray();
 private static int Max(IReadOnlyList<double> values)=>Enumerable.Range(0,values.Count).OrderByDescending(i=>values[i]).First();
 private sealed class FakeSpectrum(double[] values):IAudioSpectrumReader{internal int Reads,Disposals;internal bool Throw;public IReadOnlyList<double> ReadBands(){Reads++;if(Throw)throw new InvalidOperationException();return values;}public void Dispose()=>Disposals++;}
}
