...
internal static partial class V30SystemAuxFit
{
    private static readonly ConditionalWeakTable<TextBlock, object> Hooks = new();
    ...
    private static void Apply(Border root)
    {
        foreach (var row in Descendants(root).OfType<Grid>())
        {
            ...
            if (Hooks.TryGetValue(auxiliary, out _))
                continue;

            Hooks.Add(auxiliary, new object());
            auxiliary.LayoutUpdated += (_, _) => Fit(root, row, label, auxiliary);
        }
    }
    ...
    private static void Fit(Border root, Grid row, TextBlock label, TextBlock auxiliary)
    {
        ...
        if (width < 280 && row.ColumnDefinitions.Count >= 4)
        {
            ...
            if (label.Text == "RAM" && MemoryValue.Match(auxiliary.Text) is { Success: true } memory)
