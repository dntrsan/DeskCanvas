...
/// <summary>Pure, stable ordering for every desktop item kind. Index zero is the back-most item.</summary>
public static class LayerOrderMath
{
    public static List<CanvasItem> Normalize(IReadOnlyList<CanvasItem>? items)
    {
        ...
        var ordered = items.Select((item, index) => (item, index))
            ...
            .ThenBy(pair => pair.index) // old JSON array order is the deterministic tie breaker
    ...
    private static bool Move(IList<CanvasItem> items, CanvasItem? item, int step)
    {
        ...
        NormalizeInto(items);
        var current = IndexOf(items, item);
        if (current < 0) return false;
        var target = step switch
        {
            int.MaxValue => items.Count - 1,
            int.MinValue => 0,
            _ => Math.Clamp(current + step, 0, items.Count - 1)
        };
    ...
    public static void NormalizeInto(IList<CanvasItem> items)
    {
        var ordered = Normalize(items);
        items.Clear();
        foreach (var item in ordered) items.Add(item);
    }

    private static int IndexOf(IList<CanvasItem> items, CanvasItem item)
    {
        for (var index = 0; index < items.Count; index++) if (ReferenceEquals(items[index], item)) return index;
        return -1;
    }
}
