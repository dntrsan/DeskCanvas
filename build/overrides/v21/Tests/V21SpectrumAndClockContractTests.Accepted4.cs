...
internal static class V21SpectrumAndClockContractTestsAccepted3
{
 [ModuleInitializer] internal static void Run()
 {
  ...
  var reader=new Reader();var loop=new Loop();using(var controller=new AudioSpectrumLeaseController(()=>reader,loop)){controller.Poll(true,_=>{});if(reader.ReadCount!=1||Bool(controller,"rendering")||Value(controller,"reader") is null||loop.Count!=0)throw new InvalidOperationException("spectrum polling created an unnecessary composition-frame subscription");controller.Suspend();if(!reader.Disposed||Bool(controller,"rendering")||Value(controller,"reader") is not null||loop.Count!=0)throw new InvalidOperationException("spectrum suspend left a reader or render subscription alive");controller.Dispose();controller.Dispose();}
  ...
  var resumeFactory=new Factory();var resumeLoop=new Loop();using(var resumable=new AudioSpectrumLeaseController(resumeFactory.Create,resumeLoop)){resumable.Poll(true,_=>{});resumable.Dispose();if(resumeFactory.Created!=1||resumeFactory.Last is not {Disposed:true}||Bool(resumable,"rendering")||resumeLoop.Count!=0)throw new InvalidOperationException("spectrum dispose did not suspend the active reader");resumable.Poll(true,_=>{});if(resumeFactory.Created!=2||Bool(resumable,"rendering")||Value(resumable,"reader") is null||resumeLoop.Count!=0)throw new InvalidOperationException("spectrum did not resume without a composition-frame lease");resumable.Suspend();}
  var clock=new CanvasItem{ContentKind=CanvasContentKinds.Clock,Width=12,Height=31};if(clock.Width!=32||clock.Height!=32)throw new InvalidOperationException("Clock CanvasItem minimum size was not lowered to 32 DIP");clock.ContentKind=CanvasContentKinds.Image;if(clock.Width!=48||clock.Height!=48)throw new InvalidOperationException("non-clock CanvasItem minimum changed");if(ClockCompactLayoutMath.Scale(64,64) is <=0 or >=1||ClockCompactLayoutMath.Scale(240,160)!=1)throw new InvalidOperationException("clock compact scale contract changed");Console.WriteLine("PASS v2.1 spectrum polling avoids composition-frame load and compact clock layout");
 }
