...
// v19 keeps the v2 recovery rules while normalizing the new Now Playing options.
public sealed class LayoutRepository
{
    ...
    public CanvasLayout Load()
    {
        ...
        if (current is not null)
        {
            ...
            if (migrated)
            {
                ...
                if (knownGood is not null) MergeLegacyBuiltIns(current, knownGood);
    ...
    {
        var previous = knownGood.Items.Where(item => item.Id != Guid.Empty).GroupBy(item => item.Id).ToDictionary(group => group.Key, group => group.First());
    ...
    private static void InferLegacyBuiltIns(CanvasLayout layout)
    {
        foreach (var item in layout.Items)
        {
            ...
            if (item.ContentKind == CanvasContentKinds.Image && Path.GetExtension(item.StoredFileName).Equals(".gif", StringComparison.OrdinalIgnoreCase)) item.ContentKind = CanvasContentKinds.Gif;
        }
    }
    ...
    private static CanvasLayout Normalize(CanvasLayout layout, bool migrated)
    {
        ...
        foreach (var item in layout.Items)
        {
            ...
            if (!Enum.IsDefined(item.NowPlaying.ProgressStyle)) item.NowPlaying.ProgressStyle = NowPlayingProgressStyle.Simple;
    ...
    private static CanvasLayout? TryRead(string path)
    {
        try
        {
            ...
            foreach (var property in document.RootElement.EnumerateObject()) if (!string.Equals(property.Name, "items", StringComparison.OrdinalIgnoreCase)) properties[property.Name] = property.Value.Clone();
            ...
            var items = document.RootElement.EnumerateObject().FirstOrDefault(property => string.Equals(property.Name, "items", StringComparison.OrdinalIgnoreCase)).Value;
            if (items.ValueKind == JsonValueKind.Array)
                foreach (var entry in items.EnumerateArray())
                    try { var item = entry.Deserialize<CanvasItem>(JsonOptions); if (item is not null) layout.Items.Add(item); }
                    catch (JsonException) { }
            return layout;
        }
