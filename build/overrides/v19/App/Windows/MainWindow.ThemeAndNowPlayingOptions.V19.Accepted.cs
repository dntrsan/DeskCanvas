...
{
    private ComboBox? widgetThemeCombo;
    private TextBlock? widgetThemeLabel;
    private TextBlock? nowPlayingOptionsLabel;
    private CheckBox? spectrumVisibleCheck;
    private ComboBox? progressStyleCombo;
    ...
    private void EnsureThemeEditor()
    {
        ...
        widgetThemeCombo = new ComboBox { ItemsSource = Enum.GetValues<WidgetThemeKind>(), SelectedItem = WidgetThemeKind.Auto };
        ...
        spectrumVisibleCheck = new CheckBox { Content = "オーディオスペクトラムを表示", Margin = new Thickness(0, 2, 0, 6) };
        spectrumVisibleCheck.Checked += NowPlayingOptionChanged;
        spectrumVisibleCheck.Unchecked += NowPlayingOptionChanged;
        progressStyleCombo = new ComboBox { ItemsSource = Enum.GetValues<NowPlayingProgressStyle>(), Margin = new Thickness(0, 2, 0, 0) };
    ...
    private void UpdateThemeEditor()
    {
        ...
        progressStyleCombo.SelectedItem = isNowPlaying && item is not null && Enum.IsDefined(item.NowPlaying.ProgressStyle) ? item.NowPlaying.ProgressStyle : NowPlayingProgressStyle.Simple;
    ...
    private void NowPlayingOptionChanged(object sender, RoutedEventArgs e)
    {
        ...
        item.NowPlaying.ProgressStyle = progressStyleCombo.SelectedItem is NowPlayingProgressStyle style && Enum.IsDefined(style) ? style : NowPlayingProgressStyle.Simple;
