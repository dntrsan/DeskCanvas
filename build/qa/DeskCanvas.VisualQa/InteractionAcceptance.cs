...
internal static class InteractionAcceptance
{
    internal static void Run()
    {
        ...
        try
        {
            ...
            var controls = Descendants<System.Windows.Controls.Button>(root)
                ...
                .Append(Field<FrameworkElement>(content, "title"))
                .ToArray();
            ...
            foreach (var element in controls)
            {
                ...
                var accepted = content.IsInteractiveHit(point);
                Console.WriteLine(
                    $"QA hit {element.GetType().Name} at {point}: " +
                    $"{hit?.GetType().Name ?? "none"} => {accepted}");
                if (!accepted)
