...
internal static class InteractionContract
{
    ...
    internal static void Run()
    {
        ...
        try
        {
            ...
            if (buttons.Length != 3)
                throw new InvalidOperationException($"expected 3 transport buttons, actual {buttons.Length}");
            ...
            var moving = Heights(visualizer);
            if (frozen.SequenceEqual(moving))
            ...
            Invoke(content, "RecalculateMarquee");
            if (transform.HasAnimatedProperties)
                throw new InvalidOperationException("non-overflow title kept marquee animation");
            if (timer.IsEnabled)
                throw new InvalidOperationException("paused decoration timer kept running");
            var paused = Heights(visualizer);
            Invoke(content, "PlaybackTimer_Tick", null, EventArgs.Empty);
            if (paused.SequenceEqual(Heights(visualizer)))
                throw new InvalidOperationException("direct tick contract did not exercise bars");
