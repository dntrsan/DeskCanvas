; Isolation-only test. No CurrentVersion registry key or real Startup folder is referenced.
...
[Setup]
AppId={{5280A78F-0ACB-47D4-BC82-3E178C4C3BF2}
AppName={#AppName}
...
OutputBaseFilename=DeskCanvas-UpgradeIsolationStartupExact-{#AppVersion}
...
[Code]
const
  StartupKey = 'Software\DeskCanvas\InstallerIsolationV26\Run';
  StartupRunOnceKey = 'Software\DeskCanvas\InstallerIsolationV26\RunOnce';
  StartupApprovedRunKey = 'Software\DeskCanvas\InstallerIsolationV26\StartupApproved\Run';
  StartupApprovedRunOnceKey = 'Software\DeskCanvas\InstallerIsolationV26\StartupApproved\RunOnce';
...
begin
  ...
  begin
    ...
    begin
      if Copy(LowerCommand, Candidate, 15) = 'deskcanvas.exe' then
      begin if IsCommandEnd(LowerCommand, Candidate + 15) then begin Result := True; exit; end; end
...
var FindRec: TFindRec;
begin
  if FindFirst(ExpandConstant(MockStartupPattern), FindRec) then begin try repeat DeleteFile(ExpandConstant('{app}\legacy-startup\' + FindRec.Name)); until not FindNext(FindRec); finally FindClose(FindRec); end; end;
end;
