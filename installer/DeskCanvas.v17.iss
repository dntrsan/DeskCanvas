...
[InstallDelete]
; Never target {localappdata}\DeskCanvas: it contains user layouts, media and backups.
; The uninstaller is deliberately not listed; Inno Setup manages it during upgrade.
Type: files; Name: "{app}\DeskCanvas.VisualQa.exe"
Type: files; Name: "{app}\DeskCanvas.Tests.dll"
Type: files; Name: "{app}\DeskCanvas.*.old"
Type: filesandordirs; Name: "{app}\obsolete"
Type: filesandordirs; Name: "{app}\legacy"
Type: filesandordirs; Name: "{app}\plugins"

[Files]
; Install payload after cleanup. Same AppId keeps the normal upgrade path.
...
begin
  if (CurStep = ssPostInstall) and (not WizardIsTaskSelected('startup')) then RegDeleteValue(HKCU, StartupKey, 'DeskCanvas');
end;
procedure CurUninstallStepChanged(CurUninstallStep: TUninstallStep);
begin
  if CurUninstallStep = usUninstall then RegDeleteValue(HKCU, StartupKey, 'DeskCanvas');
end;
