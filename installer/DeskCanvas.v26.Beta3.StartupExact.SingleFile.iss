...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if IsCommandEnd(LowerCommand, Candidate + 15) then begin Result := True; exit; end;
      end
      else if Copy(LowerCommand, Candidate, 11) = 'deskcanvas-' then
      begin
        VersionStart := Candidate + 11;
        ExeOffset := Pos('.exe', Copy(LowerCommand, VersionStart, MaxInt));
        if ExeOffset > 1 then
        begin
          ExeEnd := VersionStart + ExeOffset + 3;
          if IsCommandEnd(LowerCommand, ExeEnd) then begin Result := True; exit; end;
        end;
      end;
    end;
...
begin
  ...
  for Index := 0 to GetArrayLength(Names) - 1 do
    if RegQueryStringValue(HKCU, Subkey, Names[Index], Command) and IsDeskCanvasCommand(Command) then begin Result := True; exit; end;
end;
...
begin
  ...
  for Index := 0 to GetArrayLength(Names) - 1 do
    ...
    begin RegDeleteValue(HKCU, Subkey, Names[Index]); RegDeleteValue(HKCU, ApprovedSubkey, Names[Index]); end;
end;

procedure RemoveDeskCanvasStartupShortcuts;
var FindRec: TFindRec;
begin
  if FindFirst(ExpandConstant('{userstartup}\DeskCanvas*.lnk'), FindRec) then
  begin try repeat DeleteFile(ExpandConstant('{userstartup}\' + FindRec.Name)); until not FindNext(FindRec); finally FindClose(FindRec); end; end;
end;
...
begin
  ...
  begin
    HadStartupRegistration := HasDeskCanvasRunValue(StartupKey) or HasDeskCanvasRunValue(StartupRunOnceKey) or HasPathMatch('{userstartup}\DeskCanvas*.lnk');
