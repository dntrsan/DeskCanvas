...
[Setup]
; Keep the v1.x AppId and default location: this is an in-place per-user upgrade.
...
[InstallDelete]
; Payload cleanup is deliberately confined to {app}. The Inno unins*.exe/dat files
; and all %LOCALAPPDATA%\DeskCanvas layout/media/backups stay untouched.
...
[Files]
; This follows InstallDelete, so stale payload cannot win assembly resolution.
...
begin
  ...
  begin
    { Remove a legacy executable path before copying the new in-place payload. }
